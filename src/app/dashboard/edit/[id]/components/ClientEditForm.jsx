...
export default function ClientEditForm({ post }) {
  ...
  async function handleSubmit(e) {
    ...
    if (
      ...
      toast.error("You Must make a change before submitting");
      return;
    } else {
      serverValidationText.current.textContent = "";
    }
    formData.set("tags", JSON.stringify(tags)); // Add tags to formData as a JSON string
    formData.set("postToEditStringify", JSON.stringify(post));
    ...
    <main className="u-main-container bg-white p-7 mt-32 mb-44">
      ...
      <form onSubmit={handleSubmit} className="pb-6">
        ...
        <label htmlFor="coverImage" className="f_label">
          <p>Cover Image (1280*720px maximum)</p>
          <p className="text-red-950 text-normal text-sm">
            Changing Image is optional in the edit mode
          </p>
        </label>
        <input
          type="file"
