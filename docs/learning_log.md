...
## [2026-02-18] Bug Fix: Image Loading on Vercel (Case Sensitivity)

- **Context:** Images were failing to load on the deployed Vercel site with a `400 Bad Request` error.
- **The Problem:** The `next.config.mjs` was configured to allow `techinfopullzone.b-cdn.net` (lowercase), but the actual image URLs contained `TechInfoPullZone.b-cdn.net` (mixed case). Next.js remote patterns are strict, rejecting the mismatch.
- **The Solution:** Updated the hostname pattern to `*.b-cdn.net`.
- **Key Concept:** **Remote Pattern Wildcards**. Use wildcards (e.g., `*.domain.com`) in Next.js image configuration to handle subdomain variations and case sensitivity issues robustly.
- **Code Snippet:**
  ```javascript
  // next.config.mjs
  remotePatterns: [
    {
      protocol: "https",
      hostname: "*.b-cdn.net", // Wildcard handles casing & subdomains
      pathname: "/**",
    },
  ],
  ```
